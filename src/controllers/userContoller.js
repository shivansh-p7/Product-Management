...
const updateUser = async (req, res) => {
    try {
        ...
        if (fname) {
            if (!isValidString(fname)) return res.status(400).send({ status: false, message: "Please Enter in String formate First-Name" })
            if (!/^[a-zA-Z ]{3,15}$/.test(fname)) return res.status(400).send({ status: false, message: "Please Enter Valid Last-Name" })
            final.fname = fname
        }
        ...
        if (address) {
            ...
            if (address.shipping) {
                ...
                if (street) {
                    ...
                    if (!/^[a-zA-Z0-9/]$/.test(street)) return res.status(400).send({ status: false, message: "street incorrect" })
